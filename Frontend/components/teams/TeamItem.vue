...
export default class TeamItem extends Vue {
  ...
  isAdmin() {
    return this.profile.teams.find((t: TeamInterface) => t.uuid === this.team.uuid)?.admins?.map((a: ProfileInterface) => a.uuid)?.includes(this.profile.uuid) || false;
  }

  getRole() {
    if (this.isOrgAdmin && !this.profile.teams.find((t: TeamInterface) => t.uuid === this.team.uuid)?.users?.map((u: ProfileInterface) => u.uuid)?.includes(this.profile.uuid)) {
